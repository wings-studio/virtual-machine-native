...
Value Value::Value(wolClass wolclass, SecurityModifer modifer = SecurityModifer.PRIVATE, bool isConstant = false) {
	type = wolclass;
...
Value Value::Value(wolClass wolclass, string constr_name,  Value[] arguments) : this(wolclass, SecurityModifer.PUBLIC) {
...
/// <returns></returns>
Value Value::GetField(string name) {
	try
	{
...
bool Value::CheckType(string name) = > name == type.strtype ? true : false; //thanks C# for one-string functions))
Value Value::GetSmallValue(string val, Value parent = null) {
	...
	} else if (val.StartsWith("#")) //example of syntax - set : &@this, #sum ;
	{
		...
		if (parent != null)
		{
			...
			} else
			{
				...
				else return new Value(new wolFunc(parent.GetMethod(val))); //return not  method of ParentValue by name
			}
		} else
		{
...
Value Value::GetValue(string val) {
...
/// </summary>
Value Value::VoidValue = > new Value(VirtualMachine.GetWolClass("void"));
...
/// <summary>
/// Get not  method in this value
/// </summary>
/// <param name="name">Name of not  method</param>
/// <returns></returns>
wolFunction Value::GetMethod(string name) {
	...
	if (meth.arguments.ContainsKey("this")) return meth; //check on ''
	else return null;
}
